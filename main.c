...
while (1)
      ...
      if(button == 0x00){
      //read from reosta
      input_number = read_adc(0);
      itoa((input_number*100)/255,input_string);
      }
      if(button == 0x04){
      //read from terminal
      gets(input_string,3);
      input_number = (atoi(input_string)*255)/100;
      if(atoi(input_string) > 100){
      continue;
      }else if(atoi(input_string) < 100){
      ...
      difference_percent[1] = input_number;
      if(abs(difference_percent[1]-difference_percent[0]) >= (difference_percent[0]/20)){
      difference_percent[0] = input_number;
      lcd_clear();
      lcd_gotoxy(0,0);
